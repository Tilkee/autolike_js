...
TabDriver = function () {
	...
	function TabDriver(uniqueTabId, options, client, cdpTargetId) {(0, _classCallCheck3.default)(this, TabDriver);
		...
		callback) {var _this = this;
			...
			then(function () {
				return _this.__client.Security.setOverrideCertificateErrors({ override: true });
			}).then(function () {
				if (_this.__options.whitelist.length > 0 || _this.__options.blacklist.length > 0) {
					return _this.__client.Network.setRequestInterceptionEnabled({ enabled: true });
				}
			}).then(function () {
				...
				if (_this.__options.whitelist.length > 0 || _this.__options.blacklist.length > 0) {
					var continueRequest = function continueRequest(e, error, reason) {
						if (reason && _this.__options.printAborts) {
							console.log('> Tab ' + _this.id + ': Aborted (' + reason + '): ' + e.request.url);
						}
						var payload = {
		...
		url, options, callback) {var _this3 = this;
			...
			this.__client.Page.navigate({ url: url }, function (err, res) {
				...
				if (err) {
					...
					_this3.__client.on("Network.responseReceived", responseReceived);
					var frameNavigated = function frameNavigated(e) {
						if (e.frame.id === frameId) {
							newUrl = e.frame.url;
							_this3.__client.removeListener("Network.responseReceived", responseReceived);
							_this3.__client.removeListener("Page.frameNavigated", frameNavigated);
							...
							if (_this3.__options.printNavigation) {
								console.log('> Tab ' + _this3.id + ': Navigation (open): ' + newUrl);
							}
							var domContentEventFired = function domContentEventFired() {
								_this3.__client.removeListener("Page.domContentEventFired", domContentEventFired);
								callback(null, status, statusText, newUrl);
							};
							_this3.__client.on("Page.domContentEventFired", domContentEventFired);
						}
					};
					_this3.__client.on("Page.frameNavigated", frameNavigated);
				}
			});
		...
		filename, options, callback) {var _this10 = this;
			...
			this.__client.Page.captureScreenshot(payload, function (err, res) {
		...
		name, domain, callback) {var _this19 = this;
			...
			this.__client.Network.deleteCookie(payload, function (err, res) {
		...
		} }, { key: 'client', get: function get() {return this.__client;} }, { key: 'closed', get: function get() {return this.__closed;} }, { key: 'id', get: function get() {return this.__uniqueTabId;} }]);return TabDriver;}();
